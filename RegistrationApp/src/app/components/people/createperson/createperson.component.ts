...
export class CreatePersonComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private listServices:ListService, private router: Router, private route: ActivatedRoute, private companyExistService: CompanyExistService) { }
  ...
  validation(): void {
    this.form = this.formBuilder.group({
      ...
      cpf: ['', [Validators.required, Validators.maxLength(15),Validators.minLength(11)]],
  ...
  submitForm(): void {
    if(this.form.valid){
      if(this.stateSave == 'post'){
        ...
        const personIdParam = this.route.snapshot.paramMap.get('id')
        const NewPerson= this.form.getRawValue() as Person
        this.listServices.putPerson(personIdParam as string ,NewPerson).subscribe(() => {
          this.router.navigate(['pessoas/lista'])
        }, (error) => { console.log(error) })
      }}
    }
